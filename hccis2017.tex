...
As identified in the \nameref{sec:results} section, there is a subset of 8 results which do not conform to the general trend of the data.
...
% TODO Acknowledge there is some difference between errors for different arrows - but none is significantly larger or smaller than the others and could easily have been caused by order (participant fatigue) or distractors in the environment.
...
% TODO Discuss the possibility of using the same technique but with more landmarks. More arrows could allow for user error to be minimised. Discuss that we focused on the use of 2 landmarks because it was the minimum number the technique would work with and would therefore be the quickest version of the technique with this variance (with fewer arrows to draw). Discuss future research could focus on finding a trade-off in accuracy and number of landmarks (i.e. arrows => time).
...
% TODO Remove old irrelevant references and update the relevant ones.
