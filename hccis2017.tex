...
%   noteref={n1},                        % id's of article notes, if any
   email={j.a.mcnaughton@durham.ac.uk, tcrick@cardiffmet.ac.uk, andrew.hatch@durham.ac.uk}   % All email addresses.
...
\author[
   addressref={aff1},
   email={andrew.hatch@durham.ac.uk}
...
In many co-located, collaborative systems there is a need for the constituent devices used to be aware of the physical positions of their networked counterparts.
This paper addresses this challenge by presenting a novel method of utilising users' judgement of direction to obtain the location and orientation of a touch interface.
...
\begin{keyword}
\kwd{User input, positioning, device location, touch devices, collaboration, HCI}
\end{keyword}
...
Previous research regarding collaboration on direct-touch interfaces has generally focused on the collaboration between multiple participants across a single interface~\cite{Piper2009,Rick2009,Ryall2006a}. 
However, when multiple interfaces capable of interacting with each
other are used, an opportunity arises for collaboration between users interacting with different interfaces; this use of multiple interface types in a shared environment can be beneficial for collaboration~\cite{Wallace2008a,Wallace2009}.

The physical locations of interfaces can be used to aid tasks involving interaction between networked devices. One example of this is demonstrated in the {\emph{SynergyNet}} project, a software framework built for use on direct-touch interfaces, specifically multi-touch tabletops, which allows users to perform a flicking gesture to transfer content~\cite{McNaughton2017}.
Users flick a content item in the direction of the interface to which they wish to send content; the item travels off the side of the source interface and appears on the target interface.
...
Projects such as {\emph{SynergyNet}} showcase the need for an interface to have knowledge of its location relative to networked interfaces surrounding it.
...
In order for this to be achieved the system must have information regarding the relative positions of each projected output. The stitching of multiple projector outputs to create large public displays is becoming much more common~\cite{Jones2011}; furthermore, each system which utilises the stitching of visual outputs needs a method of attaining their locations.

With systems requiring knowledge of the locations of their interfaces, a simple and effective method of obtaining this information is required.
It is possible to measure the location and orientation of an interface using physical tools such as rulers and protractors; because of the time consuming nature of this manual measurement strategy it is best suited for an environment in which the interfaces remain in a fixed position for long periods of time.
However, there are scenarios in which the interfaces may be moved on a regular basis.
For example, the {\emph{SynergyNet}} project is intended for educational environments; these classroom environments are physical spaces in which furniture is frequently moved or rearranged to accommodate different learning activities during the course of a day~\cite{Tiburcio2005}. It is therefore likely that any interfaces in the environment used by the system will not remain in fixed positions.
...
These devices are inexpensive and can be used to obtain the positional information of the object to which they are attached.
However, the accuracy of locations given by RFID chips is dependent on the number of sensors in an environment -- despite the relatively low cost of the chips, the large number of sensors needed for accurate readings can be expensive.
...
However, the technology is extremely sensitive and small changes in the ambient light level can result in the calculated location varying from the sensors' actual locations to a significant degree~\cite{Kortuem2005}.
This technology can also be affected by obstructions; an object blocking an infra-red sensor's view of one or more infra-red light sources can result in the system obtaining incorrect positional information.
Therefore, for this technology to be used in a system, the environment must be clear of obstructions and have a consistent ambient light level.
...
When attempting to obtain the location of an interface, an alternative to using sensing technologies is to utilise a technique driven by user input.
An example of this approach is where users measure the locations of the interface directly and input the information into the system through text entry. 
User input approaches have the disadvantage of relying on the accuracy of the human input, whereas sensing technologies can be relied on to be precise within certain parameters.
...
Therefore, it is important to take into consideration the accuracy of human-generated input when utilising techniques which rely on this approach.

Observations on each technique discussed in this section demonstrates that each has strengths and weaknesses.
...
For example, environments in which many users may be present around the interfaces would be unsuited to these location-inferring technologies; for example, a user may stand between the sensors used by the technologies to calculate positional information.

% table:techniquesComparison
...
For each approach discussed in this section a mark is given under each heading to which it conforms.
The significance of the list required attributes of each position obtaining technique is strengthened through their similarity to requirements often outlined for human-computer interaction~\cite{Jacob1993}, user-centered modelling~\cite{mostafa-et-al-ai2016} and natural user interface~\cite{Dietz2001} design.
...
A new approach is thus needed for scenarios such as the classroom example given in the \nameref{sec:intro} section, where accurate measurements of the interface positions must be performed quickly with many users populating the environment.
As we have demonstrated, the technique is required to be obstruction tolerant, quick, accurate, inexpensive and not encumbering.
Our approach, which potentially fulfils these requirements, is presented in the following section.


\section*{Technique}\label{sec:technique}

It is clear that the presence of users can prove to be a major disruption to many location sensing technologies; we thus seek a technique which can not only continue to work as intended with users present but can utilise their presence.
...
\textbf{$\theta$} can then be used to create a vector representing the real-world environment's y-axis locally on the interface; this can then be used in calculating the interface's physical position.

% fig:trig

After drawing the orientation arrow, the user is required to draw two further arrows which point directly to the landmarks (see Figure~\ref{fig:trig}); each of the arrows must point towards a separate landmark.
...
Furthermore, the technique does not rely on any additional technologies, so there is no extra cost for its implementation into a system.
...
The technique's design also allows for its use on interfaces which do not utilise direct-touch inputs.
...
Once a participant has finished establishing an arrow's direction they would then be expected to press a button on the interface to confirm their arrangement.

The technique first asked participants to draw a single arrow -- this corresponds to {\emph{Arrow 1}} in Figure~\ref{fig:orienTrig} -- which is used to establish the interface's orientation.
...
When a participant is satisfied with the positioning of both these arrows they are then asked to confirm their placement to complete the approach.
...
The application was designed to record the local angle of any of the arrows drawn by a participant.
...
Before the study took place, multiple direct measurements of the table's positions and orientations were made by the study organisers to confirm the true locations.
...
\begin{itemize}
\item[\textbf{Hypothesis 1:}] {\emph{The error of the technique's result is proportional to the error of user drawn arrows.}}
...
Each participant performed the technique four times -- once on each interface -- resulting in 52 instances of the technique being executed.
...
The graph in Figure~\ref{fig:scatterAverage} indicates a general trend that as the average deviation from the optimum increases, the difference between an interface's actual and calculated positions increases.
...
The evidence thus indicates that Hypothesis 2 is incorrect.


\section*{Discussion}\label{sec:discussion}

As identified in the \nameref{sec:results} section, there is a subset of eight results which do not conform to the general trend of the data.
...
The points in the graph shown in Figure~\ref{fig:scatterIndividual} represent deviation of individual participant-drawn arrows from their corresponding optimum angles for all 52 instances of the technique being performed.
...
In circumstances where the participant inaccuracy for a hypothetical inconsequential arrow is large, but is small for the other two arrows, the total deviation from the arrow's optimum angle would not be proportional to the result's error.
...
A greater confidence rating implies a higher probability of a more accurately calculated position.
...
Users were noted to take an average of 26.9 seconds per performance of the technique in the study.
...
As the accuracy of participants in the study was determined not to improve with practice, the technique may be required to be made more accurate for use in some systems.
...
By drawing additional lines to other landmarks in the environment (with known distances between them) the technique's calculations could be repeated with each pair of landmarks.
...
In this study the authors focused on the use of two landmarks as this is the minimum required for validation of this approach.
...
Once this is known, it would be possible to find the best trade-off in the additional time to draw more arrows with accuracy of the technique.

Future iterations of the study could also make other improvements on the technique's implementation; one feasible improvement could be to the method used for collecting the orientation of devices.
The instructions to participants described the first arrow as pointing to the \emph{Room's North}, this caused confusion amongst some participants and often had to be explained a number of times by study organisers.
...
It may also be possible for future studies to collect the device orientation through different approaches.
...
For this approach to work, the angle between the line from Landmark A to Landmark B and magnetic North would need to be known beforehand (much like how the technique needs to know the distance between the two landmarks beforehand).
...
Although the accuracy of in-built compasses can vary between devices, they are likely to return a more accurate result~\cite{Hwang2012} than that from the user input observed in this study.
This change to the technique is likely to vastly improve it, but would only be applicable in scenarios where the devices used have in-built compasses.
...
\begin{figure}[h!]
   \centering
   \caption{\csentence{Orientation Trigonometry.}
   Values used to calculate an interface's orientation.}
   %\includegraphics[width=0.6\textwidth]{OrientationTrigonometry.png}
   \label{fig:orienTrig}
\end{figure}

\begin{figure}[h!]
   \centering
   \caption{\csentence{Position Trigonometry.}
   Calculation of a interface's location.}
   %\includegraphics[width=0.75\textwidth]{PositionTrigonometry.png}
   \label{fig:trig}
\end{figure}

\begin{figure}[h!]
  \centering
   \caption{\csentence{Study Layout.}
   The positions and orientations of the markers and interfaces used in the user study.}
  %\includegraphics[width=0.9\textwidth]{StudyLayout.png}
   \label{fig:room}
\end{figure}

\begin{figure}[h!]
   \centering
   \caption{\csentence{Angle Deviation Scatterplot.}
   ...
   %\includegraphics[width=0.95\textwidth]{AngleDeviationScatterplot.png}
   \label{fig:scatterAverage}
\end{figure}

\begin{figure}[h!]
   \centering
   \caption{\csentence{Angle Deviation Boxplot.}
   A box plot graph showing the inaccuracy of participants against the number of times they have performed the technique.}
   %\includegraphics[width=0.75\textwidth]{AngleDeviationBoxplot.png}
   \label{fig:boxPlot}
\end{figure}

\begin{figure}[h!]
   \centering
   \caption{\csentence{Total Deviation Scatterplot.}
   ...
   %\includegraphics[width=0.95\textwidth]{TotalDeviationScatterplot.png}
